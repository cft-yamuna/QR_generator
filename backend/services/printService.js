...
const QR_MODULE_SIZE_MAX = 12; // Reduced from 16 — prevents oversized QR on Sunmi
...
export function generateEscPosBuffer(visitor, qrUrl) {
  ...
  // 2. Set line spacing: ESC 3 n — reduced to 0x10 (16 dots) from 0x1E (30 dots)
  //    Tighter spacing prevents cumulative vertical drift between repeated print jobs.
  chunks.push(Buffer.from([0x1b, 0x33, 0x10]));

  // 3. Center alignment: ESC a 1 (0x1B 0x61 0x01)
  chunks.push(Buffer.from([0x1b, 0x61, 0x01]));

  // 4. Generate ESC/POS commands for QR Code (GS ( k commands)
  ...
  // Set module size — clamped to QR_MODULE_SIZE_MAX (12) for correct Sunmi sizing
  ...
  // Print QR Code from storage
  chunks.push(Buffer.from([0x1d, 0x28, 0x6b, 0x03, 0x00, 0x31, 0x51, 0x30]));

  // 5. Single LF (0x0A) after QR code — clean, predictable one-line advance.
  //    Replaces the old Form Feed (0x0C) which caused cumulative paper drift,
  //    double-page overflow, and inconsistent position across repeated print jobs.
